...
# -----------------------------------------------------------------------------
# Test Function - missing dependencies - text_parser - line_document - case 3.
# -----------------------------------------------------------------------------
def test_missing_dependencies_text_parser_line_document_case_3(fxtr_setup_logger_environment):
    """Test Function - missing dependencies - text_parser - line_document - case 3."""
    cfg.glob.logger.debug(cfg.glob.LOGGER_START)
    ...
    try:
        ...
        del cfg.glob.line_type_header_footers

        cfg.glob.logger.debug(
            "The existing object 'cfg.glob.line_type_header_footers' of the class LineTypeHeaderFooters was deleted."
        )
    except AttributeError:
        pass
    ...
    assert expt.type == SystemExit, "Instance of class 'LineTypeHeaderFooters' is missing"
    assert expt.value.code == 1, "Instance of class 'LineTypeHeaderFooters' is missing"
...
def test_missing_dependencies_text_parser_line_document_case_4(fxtr_setup_logger_environment):
    ...
    try:
        ...
        del cfg.glob.line_type_toc

        cfg.glob.logger.debug("The existing object 'cfg.glob.line_type_toc' of the class LineTypeToc was deleted.")
    except AttributeError:
        pass

    # -------------------------------------------------------------------------
    with pytest.raises(SystemExit) as expt:
        cfg.glob.text_parser._create_line_document()

    assert expt.type == SystemExit, "Instance of class 'LineTypeToc' is missing"
    assert expt.value.code == 1, "Instance of class 'LineTypeToc' is missing"
