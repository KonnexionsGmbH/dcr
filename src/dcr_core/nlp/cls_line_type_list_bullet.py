...
class LineTypeListBullet:
    ...
    def __init__(
        ...
        dcr_core.utils.progress_msg(dcr_core.cfg.glob.setup.is_verbose_lt_list_bullet, "LineTypeListBullet")
        dcr_core.utils.progress_msg(
            dcr_core.cfg.glob.setup.is_verbose_lt_list_bullet,
    ...
    def _finish_list(self) -> None:
        ...
        if self._no_entries < dcr_core.cfg.glob.setup.lt_list_bullet_min_entries:
            dcr_core.utils.progress_msg(
                dcr_core.cfg.glob.setup.is_verbose_lt_list_bullet,
        ...
        for [page_idx, para_no, line_lines_idx_from, line_lines_idx_till] in self._entries:
            ...
            for idx in range(line_lines_idx_from, line_lines_idx_till + 1):
                ...
                if dcr_core.cfg.glob.setup.is_create_extra_file_list_bullet:
        ...
        dcr_core.utils.progress_msg(
            dcr_core.cfg.glob.setup.is_verbose_lt_list_bullet, f"LineTypeListBullet: End   list                    on page={self._page_idx+1}"
        )
    ...
    def _init_anti_patterns(self) -> list[tuple[str, re.Pattern[str]]]:
        ...
        """
        if dcr_core.cfg.glob.setup.lt_list_bullet_rule_file and dcr_core.cfg.glob.setup.lt_list_bullet_rule_file.lower() != "none":
            lt_list_bullet_rule_file_path = dcr_core.utils.get_os_independent_name(dcr_core.cfg.glob.setup.lt_list_bullet_rule_file)
            ...
            dcr_core.utils.terminate_fatal(
                f"File with bulleted list anti-patterns is missing - " f"file name '{dcr_core.cfg.glob.setup.lt_list_bullet_rule_file}'"
            )
    ...
    def _init_rules(self) -> dict[str, int]:
        ...
        if dcr_core.cfg.glob.setup.lt_list_bullet_rule_file and dcr_core.cfg.glob.setup.lt_list_bullet_rule_file.lower() != "none":
            ...
            dcr_core.utils.terminate_fatal(f"File with valid bullets is missing - " f"file name '{dcr_core.cfg.glob.setup.lt_list_bullet_rule_file}'")
    ...
    def _load_anti_patterns_from_json(
        ...
        with open(lt_list_bullet_rule_file, "r", encoding=dcr_core.cfg.glob.FILE_ENCODING_DEFAULT) as file_handle:
        ...
        dcr_core.utils.progress_msg(
            dcr_core.cfg.glob.setup.is_verbose_lt_list_bullet,
            "The bulleted list anti-patterns were successfully loaded " + f"from the file {dcr_core.cfg.glob.setup.lt_list_bullet_rule_file}",
        )
    ...
    def _load_rules_from_json(
        ...
        dcr_core.utils.progress_msg(
            dcr_core.cfg.glob.setup.is_verbose_lt_list_bullet,
            f"The list_bullet rules were successfully loaded from the file {dcr_core.cfg.glob.setup.lt_list_bullet_rule_file}",
        )
    ...
    def _process_line(self, line_line: dict[str, float | int | str]) -> None:  # noqa: C901
        ...
        if not self._entries:
            ...
            self._llx_lower_limit = round(
                ...
                * (100 - dcr_core.cfg.glob.setup.lt_list_bullet_tolerance_llx)
                / 100,
                2,
            )
            self._llx_upper_limit = round(coord_llx * (100 + dcr_core.cfg.glob.setup.lt_list_bullet_tolerance_llx) / 100, 2)
    ...
    def _process_page(self) -> None:
        """Process the page-related data."""
        dcr_core.utils.progress_msg(
            dcr_core.cfg.glob.setup.is_verbose_lt_list_bullet, f"LineTypeListBullet: Start page                           ={self._page_idx + 1}"
        )
        ...
        dcr_core.utils.progress_msg(
            dcr_core.cfg.glob.setup.is_verbose_lt_list_bullet, f"LineTypeListBullet: End   page                           ={self._page_idx + 1}"
        )
    ...
    def _reset_document(self) -> None:
        """Reset the document memory."""
        dcr_core.utils.progress_msg(dcr_core.cfg.glob.setup.is_verbose_lt_list_bullet, "LineTypeListBullet: Reset the document memory")
    ...
    def _reset_list(self) -> None:
        ...
        dcr_core.utils.progress_msg(dcr_core.cfg.glob.setup.is_verbose_lt_list_bullet, "LineTypeListBullet: Reset the list memory")
    ...
    def process_document(
        ...
        if dcr_core.cfg.glob.setup.is_create_extra_file_list_bullet and self._lists:
            full_name = dcr_core.utils.get_full_name(
                directory_name,
                dcr_core.utils.get_stem_name(str(file_name_curr)) + "_list_bullet." + dcr_core.cfg.glob.FILE_TYPE_JSON,
            )
        ...
        if self.no_lists > 0:
            dcr_core.utils.progress_msg(
                dcr_core.cfg.glob.setup.is_verbose_lt_list_bullet, f"LineTypeListBullet:                 number bulleted lists={self.no_lists}"
            )
