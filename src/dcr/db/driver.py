...
def connect_db() -> None:
    ...
    comm.utils.progress_msg_connected()
...
def create_database() -> None:
    ...
    if cfg.glob.setup.db_dialect == cfg.glob.DB_DIALECT_POSTGRESQL:
        ...
        comm.utils.terminate_fatal(f"A database dialect '{cfg.glob.setup.db_dialect}' " f"is not supported in DCR")
...
def create_database_postgresql() -> None:
    ...
    comm.utils.progress_msg(f"The user '{user}' has now been created")

    cfg.glob.db_driver_cur.execute("CREATE DATABASE " + database + " WITH OWNER " + user)
    comm.utils.progress_msg("The database '{database}' has now been created")

    cfg.glob.db_driver_cur.execute("GRANT ALL PRIVILEGES ON DATABASE " + database + " TO " + user)
    comm.utils.progress_msg("The user '{user}' has now all privileges on database '{database}'")
    ...
    comm.utils.progress_msg(
        f"The database has been successfully created, " f"version number='{cfg.glob.setup.dcr_version}"
    )
...
def disconnect_db() -> None:
    ...
    comm.utils.progress_msg_disconnected()
...
def drop_database_postgresql() -> None:
    ...
    comm.utils.progress_msg(f"If existing, the database '{database}' has now been dropped")

    cfg.glob.db_driver_cur.execute("DROP USER IF EXISTS " + user)
    comm.utils.progress_msg(f"If existing, the user '{user}' has now been dropped")
...
def upgrade_database() -> None:
    ...
    comm.utils.progress_msg("Upgrade the database tables ...")

    current_version: str = db.dml.select_version_version_unique()

    if current_version == cfg.glob.setup.dcr_version:
        comm.utils.progress_msg(f"The database is already up to date, version number='{current_version}'")
    else:
